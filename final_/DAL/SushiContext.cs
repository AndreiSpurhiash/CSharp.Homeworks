...
namespace Final_project.DAL
{
    public class SushinContext : DbContext
    {
        ...
        {
            optionsBuilder.UseNpgsql("Host=localhost;Port=5432;Database=pilot;Username=postgres;Password=1234;");
        }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Basket>(entity =>
            {
                ...
                entity.HasKey(x => x.Id).HasName("Id3");
                entity.Property(x => x.GuidOrder).HasColumnName(@"GuidOrder").HasColumnType("uuid").IsRequired();
                entity.Property(x => x.NameSushi).HasColumnName(@"NameSushi").HasColumnType("text").IsRequired();
                entity.Property(x => x.Count).HasColumnName(@"Count").HasColumnType("int").IsRequired();
                entity.HasOne(a => a.Order).WithOne(b => b.Basket).OnDelete(DeleteBehavior.Cascade);
                entity.HasMany(a => a.Sushi).WithOne(b => b.Basket).HasForeignKey(c => c._Number).OnDelete(DeleteBehavior.Cascade);
